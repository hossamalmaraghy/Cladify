  ...
  const loadOrderData = async () => {
    try {
      ...
      if (response.data.success) {
        // Try to get orders from response.data.orders or response.data.Orders
        const orders = response.data.orders || response.data.Orders;
        if (!orders) {
          console.error("Orders data not found in response", response.data);
          return;
        }

        let allOrdersItem = [];
        orders.map((order) => {
          // Make sure order.items exists
          if (order.items && Array.isArray(order.items)) {
            order.items.map((item) => {
              // Merge order-level details into each item
